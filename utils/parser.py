    ...
    for i in range(n):
        while True:
            try:
                ...
                if len(baris) != n:
                    print(
                        f"    Error: Harap masukkan tepat {n} angka. Anda memasukkan {len(baris)}."
                    )
                else:
...
def parse_vektor_interaktif(n):
    ...
    while True:
        try:
            str_vektor = input("    Vektor: ").replace(",", " ")
            vektor = [float(angka) for angka in str_vektor.split()]

            if len(vektor) != n:
                print(
                    f"    Error: Harap masukkan tepat {n} angka. Anda memasukkan {len(vektor)}."
                )
            else:
